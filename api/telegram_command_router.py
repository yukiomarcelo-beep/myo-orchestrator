...
class TelegramAPI:
    ...
    def send(self, chat_id: int | str, text: str, parse_mode: str = "HTML") -> None:
...
class TelegramCommandRouter:
        ...
        if not message:
            ...
            if not chat_id or not text:
                ...
                if not handler:
                    if text.startswith("/"):
                        self.api.send(
                            chat_id, "Comando não reconhecido. Use /ajuda para ver os disponíveis."
                        )
    ...
    def _handle_ajuda(self, chat_id: int, text: str) -> None:
        ...
        for cmd, desc in self.COMMANDS.items():
            ...
            lines.append("\n<b>Exemplos:</b>")
            lines.append(
                "<code>/scan_dores restaurant|profit margin|restaurantowners,smallbusiness</code>"
            )
            lines.append("<code>/scan_concorrencia restaurant|profit margin</code>")
            lines.append(
                "<code>/criar_produto restaurant|profit margin pricing|restaurantowners,entrepreneur</code>"
            )
    ...
    def _handle_scan_dores(self, chat_id: int, text: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if not complaints:
                    self.api.send(
                        chat_id, " Nenhuma reclamação encontrada. Tente outros subreddits."
                    )
                    return
    ...
    def _handle_criar_produto(self, chat_id: int, text: str) -> None:
        ...
        """
        try:
            niche, problem, subreddits = self._parse_three_part_command(text, "/criar_produto")
        except ValueError as e:
            self.api.send(chat_id, f" {e}")
            return
    ...
    def _build_complaints_summary(self, payload: List[Dict[str, Any]]) -> str:
        if not payload:
    ...
    def _build_competitors_summary(self, payload: List[Dict[str, Any]]) -> str:
        if not payload:
    ...
    @staticmethod
    def _parse_two_part_command(text: str, command: str) -> tuple[str, str]:
    ...
    @staticmethod
    def _parse_three_part_command(text: str, command: str) -> tuple[str, str, List[str]]:
        content = text.replace(command, "", 1).strip()
        parts = [p.strip() for p in content.split("|")]
        if len(parts) < 3:
            raise ValueError(f"Use: {command} &lt;nicho&gt;|&lt;problema&gt;|&lt;sub1,sub2&gt;")
