...
async def sugerir_estrategia():
 try:
  os.chdir(str(BASE_DIR))
  from strategic_memory import sugerir_melhor_estrategia_completo, init_strategic_db
  init_strategic_db()
...
async def trocar_estrategia(body: EstrategiaBody):
 if body.objetivo not in OBJETIVOS_VALIDOS:
  ...
  if body.fase not in FASES_VALIDAS:
   ...
   try:
    os.chdir(str(BASE_DIR))
    from strategic_memory import (registrar_estrategia, pode_mudar_estrategia,
     ...
     if ultimo["lucro"] < 0:
      ...
      if len(result) >= 3:
       ...
       if ultimos[2]["lucro"] < ultimos[1]["lucro"] < ultimos[0]["lucro"]:
        ...
        try:
         from llm_router import get_monthly_cost_usd
