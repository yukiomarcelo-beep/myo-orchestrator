...
from engines.execution_engine import ExecutionEngine, ExecutionTask
    ...
    try:
        with open(FEEDBACK_JSON, encoding="utf-8") as f:
            ...
            if not rch:
                ...
                rch_map = {
                    "policy_rigidity": "topic",
                    "agent_inference": "engine",
        ...
        try:
            ...
            if ctx_filter:
                tasks = [
                    t
                    for t in tasks
                    if ctx_filter.lower() in str(t.get("context", {}) or t.get("tags", [])).lower()
                ]
...
def handle_status(ctx_filter: str = ""):
    ...
    if not tasks:
        msg = (
            f"Nenhuma task para contexto '{ctx_filter}'."
            if ctx_filter
            else "Nenhuma task encontrada."
        )
        print(f" {msg}")
        input("\n Enter para voltar...")
        return
                    ...
                    if t.get("issue_url"):
                        ...
                        if status in STATUS_FAILED:
                            ...
                            if hint:
                                ...
                                if ctx_filter:
                                    print(
                                        f" (filtro ativo: context='{ctx_filter}' | 0 para limpar)"
                                    )
                                else:
                                    ...
                                    failed_visible = [
                                        t
                                        for t in tasks[:15]
                                        if t.get("status", "") in STATUS_FAILED
                                    ]
                                    if failed_visible:
                                        print(
...
def _task_details(t: dict):
    ...
    for f in fields:
        ...
        if val is not None:
            ...
            if hint:
                freq = (
                    f" ({count}x — frequente)" if count >= 3 else (f" ({count}x)" if count else "")
                )
...
def _discard_task(t: dict):
    ...
    print(f"\n Task: {t.get('title','—')[:55]}")
    print("\n Motivo do descarte:")
    print(" 1. Irrelevante (ideia descartada)")
    print(" 2. Bloqueada por policy (falhou demais)")
    print(" 3. Decisão minha (mudei de plano)")
    print(" 0. Cancelar")
                            ...
                            try:
                                task = ExecutionTask(
                                    **{
                                        ...
                                        if k in ExecutionTask.__dataclass_fields__
                                    }
                                )
                                ...
                                executor = getattr(result, "assigned_agent", None) or getattr(
                                    result, "execution_channel", "—"
                                )
                                dispatched_at = getattr(result, "dispatched_at", None) or getattr(
                                    result, "updated_at", "—"
                                )
                                error = getattr(result, "dispatch_error", None) or getattr(
                                    result, "error_message", None
                                )
                                attempts = getattr(result, "attempts", None)

                                succeeded = (
                                    "complet" in str(status).lower() or status == "dispatched"
                                )
                                ...
                                if attempts is not None:
                                    ...
                                    if error:
                                        ...
                                        if (
                                            t_data.get("retry_mode")
                                            and t_data.get("experiment_result") is None
                                        ):
                                            t_data["experiment_result"] = (
