...
from observability import tracer, tracker
...
def add_product_cost(raw: dict) -> dict:
    ...
    idx = next(
        (
            i
            for i, p in enumerate(data["product_financials"])
            if p.get("idea_title") == record["idea_title"]
        ),
        None,
    )
    if idx is not None:
...
def _auto_decisions(
    monthly_revenue: float, monthly_cost: float, monthly_profit: float, monthly_margin: float
) -> list:
    decisions = []
...
def show_dashboard():
    ...
    if products:
        ...
        for p in sorted(products, key=lambda x: -x.get("margin_pct", 0)):
            ...
            print(
                f"  {ic} {p.get('idea_title',''):<30}  margem {mg:5.1f}%  "
    ...
    if not decisions:
        ...
        decisions = _auto_decisions(
            monthly_revenue, monthly_cost, monthly_profit, latest.get("monthly_margin", 0)
        )
    if decisions:
        ...
        for d in decisions:
            print(
                f"  {ICON.get(d.get('priority','media'),'')} [{d.get('trigger','')}] {d.get('action','')}"
            )
...
async def main_async():
    ...
    if args.product:
        ...
        if plano:
            ...
            for m in plano:
                print(
                    f"  Mês {m.get('mes','')}: {m.get('foco','')} — meta R${m.get('meta_receita',0):,.0f}"
                )
