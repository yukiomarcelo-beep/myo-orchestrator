...
_OBJETIVO_BOOST: dict = {
    "maximizar_receita": {
        ...
        "conversao_copy": 1.5,
        "fluxo_venda": 1.4,
        "engajamento_conteudo": 1.2,
        "margem_financeira": 1.3,
    },
    "reduzir_custo": {
        "custo_api": 1.7,
        "erro_operacional": 1.5,
        "margem_financeira": 1.4,
...
_FASE_EXPLORACAO: dict = {
    "validacao": [
        {
            "tipo": "conversao_copy",
            "descricao": "Estratégia/validacao: testar nova copy de conversão",
            "impacto": 8,
            "frequencia": 3,
            "urgencia": 7,
            "complexidade": 2,
            "fonte": "strategic_kaizen",
        },
        {
            "tipo": "fluxo_venda",
            "descricao": "Estratégia/validacao: simplificar funil de vendas",
            "impacto": 7,
            "frequencia": 2,
            "urgencia": 6,
            "complexidade": 3,
    ...
    "escala": [
        {
            ...
            "descricao": "Estratégia/escala: aumentar alcance de conteúdo",
            "impacto": 8,
            "frequencia": 3,
            "urgencia": 7,
            "complexidade": 2,
        ...
        {
            "tipo": "conversao_copy",
            "descricao": "Estratégia/escala: otimizar CTA para volume",
            "impacto": 7,
            "frequencia": 3,
            "urgencia": 6,
            "complexidade": 2,
    ...
    "lucro": [
        {
            "tipo": "custo_api",
            "descricao": "Estratégia/lucro: cortar custo de API sem perda de qualidade",
            "impacto": 8,
            "frequencia": 2,
            "urgencia": 7,
            "complexidade": 2,
        ...
        },
        {
            ...
            "descricao": "Estratégia/lucro: reajustar pricing para maximizar margem",
            "impacto": 9,
            "frequencia": 2,
            "urgencia": 8,
            "complexidade": 3,
    ...
    "produto": [
        {
            ...
            "descricao": "Estratégia/produto: revalidar padrões com dados frescos",
            "impacto": 7,
            "frequencia": 2,
            "urgencia": 6,
            "complexidade": 3,
        ...
        },
        {
            ...
            "descricao": "Estratégia/produto: eliminar erros recorrentes no pipeline",
            "impacto": 8,
            "frequencia": 2,
            "urgencia": 7,
            "complexidade": 3,
...
EXPLORACAO_CANDIDATOS = [
    {
        "tipo": "conversao_copy",
        "descricao": "Exploração autônoma: testar nova abordagem de copy/CTA",
        "impacto": 6,
        "frequencia": 3,
        "urgencia": 5,
        "complexidade": 2,
        "fonte": "exploracao_autonoma",
    },
    {
        "tipo": "preco_oferta",
        "descricao": "Exploração autônoma: testar variação de preço ou bundle",
        "impacto": 7,
        "frequencia": 2,
        "urgencia": 4,
        "complexidade": 3,
    ...
    {
        "tipo": "fluxo_venda",
        "descricao": "Exploração autônoma: testar simplificação do funil de venda",
        "impacto": 6,
        "frequencia": 2,
        "urgencia": 4,
        "complexidade": 3,
    ...
    {
        "tipo": "engajamento_conteudo",
        "descricao": "Exploração autônoma: testar novo formato/gancho de conteúdo",
        "impacto": 5,
        "frequencia": 3,
        "urgencia": 3,
        "complexidade": 2,
    ...
    {
        "tipo": "custo_api",
        "descricao": "Exploração autônoma: testar otimização de modelo IA",
        "impacto": 5,
        "frequencia": 2,
        "urgencia": 3,
        "complexidade": 2,
        ...
        try:
            with open(FINANCIAL_FILE, encoding="utf-8") as f:
                ...
                if len(projs) >= 2:
                    last = projs[-1]
                    prev = projs[-2]
                    margin_drop = prev.get("monthly_margin_pct", 0) - last.get(
                        "monthly_margin_pct", 0
                    )
                    if margin_drop > 5:
                        problemas.append(
                            {
...
def verificar_estagnacao(conn: sqlite3.Connection) -> Optional[str]:
    ...
    row = conn.execute(
        "SELECT COUNT(*) as n FROM kaizen_history WHERE aplicado=1 AND applied_at >= ?", (cutoff,)
    ).fetchone()
        ...
        with open(rec_path, "a", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {
                        "ts": _now(),
                        "tipo": "stagnation_alert",
                        "mensagem": alerta,
                        "melhorias_7d": melhorias_7d,
                    }
                )
                + "\n"
            )
...
def registrar_estrategia(
    conn: sqlite3.Connection, objetivo: str, fase: str, motivo: str = "manual"
) -> int:
...
def verificar_mudanca_permitida(conn: sqlite3.Connection) -> Tuple[bool, int]:
    ...
    row = conn.execute("SELECT ts FROM strategic_history ORDER BY ts DESC LIMIT 1").fetchone()
    if not row:
        return True, 999
        try:
...
def sugerir_melhoria_rules(problema: Dict) -> Dict:
    ...
    sugestoes = _RULES_SUGESTOES.get(
        problema["tipo"],
        [
            {
        ...
        try:
            ...
            response = httpx.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "user", "content": f"Problema detectado no pipeline MYO:\n{ctx}"}
                    ],
                },
                timeout=30.0,
            )
...
def aplicar_em_producao(
    conn: sqlite3.Connection, kaizen_id: int, melhoria: Dict, problema: Dict
) -> bool:
    """
...
def registrar_kaizen(
    conn: sqlite3.Connection, ciclo: int, problema: Dict, melhoria: Dict, sandbox: Dict
) -> int:
...
def registrar_resultado(
    conn: sqlite3.Connection, kaizen_id: int, resultado: str, score: float = 0.0
):
    conn.execute(
...
def iniciar_ab_test(
    conn: sqlite3.Connection, kaizen_id: int, tipo: str, variante_a: str, variante_b: str
) -> int:
...
def atualizar_ab_test(
    conn: sqlite3.Connection, ab_id: int, exec_a: int, exec_b: int, sucesso_a: int, sucesso_b: int
):
    conn.execute(
        """UPDATE ab_tests
...
class KaizenEngine:
    ...
    def _get_last_ciclo(self) -> int:
        row = self.conn.execute("SELECT MAX(ciclo) as n FROM kaizen_history").fetchone()
        return row["n"] or 0
    ...
    def run_cycle(self) -> Dict:
        ...
        if explorar:
            ...
            print(
                f" Exploração estratégica [{FASE_NEGOCIO}/{OBJETIVO_ATUAL}]: {candidato['descricao']}"
            )

            if not problemas:
                print(" Nenhum problema encontrado — sistema saudável")
                return {"ciclo": self.ciclo, "problemas": 0, "aplicadas": 0}

                print(f" {len(problemas)} problema(s) encontrado(s)")

                # 2. Priorizar
                print(" [2/4] Priorizando...")
                ...
                for i, p in enumerate(priorizados[:5], 1):
                    ...
                    # 3. Loop de melhoria
                    print(" [3/4] Gerando e testando melhorias...")
                    aplicadas = 0
                    ...
                    for problema in priorizados:
                        if aplicadas >= MAX_MELHORIAS_POR_CICLO:
                            ...
                            print(
                                f" Tipo: {problema['tipo']} | Score: {problema['score']:.1f} | "
                            ...
                            print(
                                f" {src_icon} Sugestão ({melhoria['fonte_sugestao']}): {melhoria['acao']}"
                            )
                            if melhoria.get("detalhe"):
                                ...
                                print(
                                    f" {sb_icon} Sandbox: impacto estimado {sandbox['impacto']:.2f}× "
                                    f"(conf {sandbox['confianca']:.0%}, "
                                    f"histórico: {sandbox['n_historico']} casos)"
                                )

                                # Registrar
                                kaizen_id = registrar_kaizen(
                                    self.conn, self.ciclo, problema, melhoria, sandbox
                                )

                                if sandbox["sucesso"]:
                                    # Aplicar
                                    ok = aplicar_em_producao(
                                        self.conn, kaizen_id, melhoria, problema
                                    )
                                    if ok:
                                        ...
                                        if problema["tipo"] in (
                                            ...
                                            print(
                                                " Melhoria não aprovada no sandbox — registrada para revisão"
                                            )
                                            ...
                                            print(f"\n {''*60}")
                                            print(
                                                f" Ciclo finalizado: {len(problemas)} problemas, "
                                            ...
                                            if alerta_estagnacao:
                                                ...
                                                if (
                                                    sugestao
                                                    and sugestao["objetivo"] != OBJETIVO_ATUAL
                                                ):
...
def show_history(limit: int = 20):
    if not DB_FILE.exists():
        ...
        print(
            f" {'ID':<4} {'Ciclo':<6} {'Tipo':<24} {'Fonte':<7} {'Sandbox':<8} {'Apl':<4} {'Resultado'}"
        )
        print(f" {''*80}")
        for r in rows:
            icon = "" if r["aplicado"] else "⏸"
            res = {"melhorou": "", "piorou": "", "neutro": "", "pending": "⏳"}.get(
                r["resultado_real"], "?"
            )
            print(
                f" {r['id']:<4} #{r['ciclo']:<5} {r['tipo']:<24} "
...
def show_ab_report():
    if not DB_FILE.exists():
        ...
        print(
            f" {'ID':<4} {'Tipo':<24} {'Status':<12} {'Winner':<8} "
        ...
        for r in rows:
            ...
            print(
                f" {r['id']:<4} {r['tipo']:<24} {r['status']:<12} {winner:<8} "
...
def main():
    ...
    parser.add_argument("--loop", action="store_true", help="Loop contínuo (padrão: 1 ciclo e sai)")
    parser.add_argument("--status", action="store_true", help="Estado atual do banco Kaizen")
    parser.add_argument("--history", action="store_true", help="Histórico de melhorias registradas")
    parser.add_argument("--ab", action="store_true", help="Relatório de A/B tests em andamento")
    parser.add_argument(
        "--interval",
        type=int,
