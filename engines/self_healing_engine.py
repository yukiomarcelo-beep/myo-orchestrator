...
def upsert_event(conn, fingerprint: str, source: str, event_type: str,
severity: str, details: dict, product: str = "") -> Optional[int]:
 """Insere evento se ainda não existe (por fingerprint). Retorna id ou None."""
 existing = conn.execute(
 "SELECT id, status, attempts FROM operational_events WHERE fingerprint=?",
 (fingerprint,)
 ).fetchone()
 if existing:
  ...
  if existing["status"] in ("resolved", "ignored"):
   ...
   VALUES (?,?,?,?,?,?,?,?,?)""",
   (fingerprint, source, event_type, severity, product,
   json.dumps(details, ensure_ascii=False), "detected", _now(), _now())
   )
   conn.commit()
...
def add_history(conn, event_id: int, attempt: int, action: str,
before: dict, after: dict, success: bool, notes: str = ""):
 conn.execute(
 """INSERT INTO correction_history
 (event_id, attempt, action, before_json, after_json, success, notes, ts)
 VALUES (?,?,?,?,?,?,?,?)""",
 (event_id, attempt, action, json.dumps(before), json.dumps(after),
 int(success), notes, _now())
 )
 conn.commit()
...
def analyze(self, event_type: str, severity: str, details: dict,
product: str = "") -> dict:
 if not ANTHROPIC_KEY:
  ...
  "active_engines": ["opportunity_scorer","product_engine","content_engine","autonomous_agent"],
  ...
  try:
   ...
   def _fallback_diagnosis(self, event_type: str, severity: str,
   details: dict) -> dict:
    ...
    "acao_recomendada": "recalibrate",
    "parametros_acao": {"action": "run_audit", "engine": details.get("worst_engine","")},
    "urgencia_minutos": 60,
...
class CorrectionAgent:
 ...
 def apply(self, acao: str, parametros: dict, details: dict,
 event_id: int) -> tuple[bool, str, dict, dict]:
  handler = {
  ...
  if TASKS_DIR.exists():
   for f in TASKS_DIR.glob("*.json"):
    try:
     ...
     if task.get("status") in ("failed", "error") and task.get("attempts", 0) < max_r:
      task["status"] = "pending"
 ...
 def _restart_engine(self, params, details, event_id) -> tuple:
  ...
  if engine in ("verification_engine", "decision_logger"):
   ...
   after = self._snapshot_metrics()
   return ok, f"restart:{engine}:decision_logger_rebuild_{'ok' if ok else 'erro'}", before, after
 ...
 def _check_api(self, params, details, event_id) -> tuple:
  ...
  if missing:
   ...
   "Verificar .env e reconfigurar.", "critical"
   )
 ...
 def _check_confidence(self, before, after) -> tuple:
  ...
  if a == 0:
   ...
   if delta > 2:
    ...
    if delta < -2:
     return False, max(0.1, 0.5 + delta / 20), f"confiança CAIU mais {-delta:.1f} pts — "
...
class ExplainabilityLayer:
 ...
 def record(self, conn: sqlite3.Connection, event_id: int,
 diagnosis: dict, fonte: str,
 overridden: bool = False, learned_action: str = "") -> dict:
  ...
  if overridden and learned_action and learned_action != acao:
   ...
   if not alternativas:
    alternativas = [{"acao": "notify_human",
    ...
    VALUES (?,?,?,?,?,?,?,?)""",
    (event_id, causa, decisao,
    json.dumps(alternativas, ensure_ascii=False),
    confianca, fonte, int(overridden), _now())
    )
    conn.commit()
 ...
 def get(self, conn: sqlite3.Connection, event_id: int) -> Optional[dict]:
  row = conn.execute(
  "SELECT * FROM explanation_log WHERE event_id=? ORDER BY ts DESC LIMIT 1",
  (event_id,)
  ).fetchone()
  if not row:
   return None
...
class SafetyGuard:
 ...
 def can_apply(self, action: str, event_type: str = "") -> tuple[bool, str]:
  ...
  if self._circuit_open:
   ...
   "SELECT COUNT(*) FROM action_audit_log WHERE action=? AND ts>?",
   (action, cutoff)
   ).fetchone()[0]

   if count >= limit:
    return False, (
    f"rate_limit:{action}:{count}/{limit}_por_hora"
    " — aguardar próximo ciclo"
    )

    return True, "ok"

 def record(self, action: str, event_type: str, success: bool,
...
class LearningLayer:
 ...
 def record_outcome(self, event_type: str, action: str,
 success: bool, validation_score: float,
 human_validated: bool = False):
  ...
  if existing:
   ...
   if human_validated and success and new_hvc >= 2:
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?)""",
    (event_type, action, 1, int(success),
    init_rate, init_rate, validation_score, hvc_delta,
    "alto" if human_validated and success else "normal", _now())
    )
    self.conn.commit()
 ...
 def print_table(self):
  ...
  print(f"\n {''*85}")
  print(f" Patterns Learned — Memória de Correções (com peso humano)")
  print(f" {''*85}")
  if not patterns:
   print(" Nenhum padrão aprendido ainda (mínimo 3 tentativas por tipo).")
   return
   print(f" {'Tipo de Evento':<28} {'Melhor Ação':<20} {'Tent':>5} {'Sucesso':>8} "
...
_FINANCIAL_TYPES = {"financial_anomaly", "revenue_drop", "financial_data_stale",
"revenue_risk", "cost_spike"}
_REVENUE_AFFECTING = {"high_block_rate", "confidence_drop", "pipeline_stalled",
"financial_anomaly", "revenue_drop", "engine_bias_detected"}
_USER_AFFECTING = {"pipeline_stalled", "task_failures", "engine_stalled",
"api_failure", "stale_pipeline"}
_STRUCTURAL_ACTIONS = {"relax_threshold", "tighten_threshold", "restart_engine",
"recalibrate"}


def calcular_impacto(ev_type: str, acao: str, attempts: int,
details: dict) -> dict:
 """
...
class HumanApprovalGate:
 ...
 def request_approval(self, event_id: int, ev_type: str, impacto: dict,
 diagnosis: dict, acao: str, exp: dict) -> int:
  """Persiste solicitação e envia alerta Telegram. Retorna approval_id."""
  causa = diagnosis.get("causa_provavel", "—")
  risco = diagnosis.get("risco_inacao",
  "Sistema pode degradar progressivamente sem intervenção.")
  ...
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
  (event_id, ev_type,
  impacto["nivel"], impacto["score"],
  json.dumps(impacto["fatores"]),
  acao, causa[:500], risco[:500], fallback,
  "pending", _now(), timeout_at)
  )
  self.conn.commit()
 ...
 def wait_for_response(self, approval_id: int,
 timeout: Optional[int] = None) -> tuple[str, str]:
  ...
  print(f" ⏳ Aguardando aprovação #{approval_id} "
  f"(timeout: {timeout or self.TIMEOUT_SECONDS}s)...")

  while time.time() < deadline:
   row = self.conn.execute(
   "SELECT status, acao_ajustada FROM human_approvals WHERE id=?",
   (approval_id,)
   ).fetchone()
   ...
   if row and row["status"] != "pending":
    ...
    "adjusted": ""}.get(status, "•")
    print(f" {icon} Resposta humana recebida: {status}"
    ...
    row = self.conn.execute(
    "SELECT safe_fallback FROM human_approvals WHERE id=?",
    (approval_id,)
    ).fetchone()
 ...
 def respond(self, approval_id: int, status: str,
 acao_ajustada: str = "", motivo: str = "") -> bool:
  """Chamado pelos flags --approve / --reject / --adjust."""
  row = self.conn.execute(
  "SELECT id, status FROM human_approvals WHERE id=?",
  (approval_id,)
  ).fetchone()
  if not row:
   ...
   if row["status"] != "pending":
    ...
    print(f" {icon} Aprovação #{approval_id} → {status}"
 ...
 def show_pending(self):
  items = self.pending()
  print(f"\n {''*65}")
  print(f" Human-in-the-Loop — Aprovações Pendentes")
  print(f" {''*65}")
  if not items:
   print(" Nenhuma aprovação pendente.\n")
   return
   for a in items:
    print(f" #{a['id']} [{a['impact_nivel']} score:{a['impact_score']}] "
    f"{a['ev_type']}")
 ...
 def _send_telegram_alert(self, approval_id: int, ev_type: str, impacto: dict,
 causa: str, acao: str, risco: str, fallback: str):
...
class HumanDecisionIntelligence:
 ...
 def record_decision(self, approval_id: int, event_id: int, ev_type: str,
 decisao: str, acao_original: str, acao_final: str,
 impact_nivel: str, impact_score: int,
 motivo: str = "") -> int:
  """
  Grava decisão humana. Detecta contradição com padrão.
  Retorna decision_id.
  """
  contradicts, pattern_action = self._check_contradiction(
  ev_type, acao_original, acao_final
  )
  ...
  pattern_action, ts)
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
  (approval_id, event_id, ev_type, decisao,
  acao_original, acao_final or "",
  motivo, impact_nivel, impact_score,
  int(contradicts), pattern_action, _now())
  )
  self.conn.commit()

  if contradicts:
   print(f" [HDI] Divergência detectada: sistema→{acao_original}, "
 ...
 def record_outcome(self, approval_id: int, resultado: str,
 validation_score: float, learning: 'LearningLayer'):
  """
  ...
  if not row:
   ...
   if row["acao_final"]:
    ...
    row["ev_type"], row["acao_final"],
    success, validation_score,
 ...
 def _check_contradiction(self, ev_type: str, acao_sistema: str,
 acao_humano: str) -> tuple[bool, str]:
  ...
  if not acao_humano or acao_sistema == acao_humano:
   ...
   try:
    ...
    if not row:
     return False, ""
     if (row["success_rate"] >= self.HIGH_CONFIDENCE_THRESHOLD
     and row["best_action"] != acao_humano):
      return True, row["best_action"]
      return False, ""

 def _register_successful_contradiction(self, ev_type: str, acao_padrao: str,
 acao_humano: str, validation_score: float,
 ...
 def get_stats(self) -> dict:
  ...
  try:
   ...
   "taxa_acerto_sistema": round(by_decisao.get("approved", 0) / total, 3) if total > 0 else 0.0,
   "validadas": validadas,
 ...
 def print_report(self):
  ...
  if not s:
   ...
   return
   print(f"\n {''*65}")
   print(f" Human Decision Intelligence — Relatório")
   print(f" {''*65}")
   ...
   print(f" Taxa de divergência : {s['taxa_divergencia']:.0%} "
   f"(divergência = rejeições + ajustes)")
   print(f" Acerto do sistema : {s['taxa_acerto_sistema']:.0%} "
   f"(sistema estava certo sem intervenção)")
   print(f" Contradições/padrão: {s['contradicoes']}")
   if s["validadas"]:
    print(f" Sucesso pós-decisão: {s['taxa_sucesso_final']:.0%} "
    f"({s['validadas']} decisões com resultado validado)")
    if s["recentes"]:
     print(f"\n Últimas decisões:")
     print(f" {'Tipo':<24} {'Decisão':<10} {'Original':<18} {'Final':<16} {'Resultado'}")
     print(f" {''*24} {''*10} {''*18} {''*16} {''*10}")
     for r in s["recentes"]:
      div = "" if r["contradicts_pattern"] else " "
      print(f" {div}{r['ev_type']:<23} {r['decisao']:<10} "
      ...
      try:
       ...
       if weighted:
        ...
        for p in weighted:
         ...
         print(f" {icon} {p['event_type']:<28} → {p['best_action']:<20} "
...
class ConfidenceGovernance:
      ...
      if days < 1.0:
       ...
       if old_adj is None:
        ...
        if abs(new_adj - old_adj) < 0.005:
         ...
         self._log_event(row["event_type"], row["best_action"], "decay",
         old_adj, new_adj, {"days_idle": round(days, 1), "raw_decay": decay})
         affected.append({
         "event_type": row["event_type"], "action": row["best_action"],
         "old": old_adj, "new": new_adj, "days": round(days, 1),
         })
       ...
       try:
        ...
        if rec and rec["total"] >= 2:
         ...
         if recent_rate < hist_rate - 0.20:
          failures += 1
          notes.append(f"degradação: {recent_rate:.0%} vs histórico {hist_rate:.0%}")

          old_adj = p.get("confidence_adjusted") or p.get("success_rate") or 1.0
          new_adj = old_adj
          ...
          if failures >= 2:
           ...
           new_adj = round(max(self.DECAY_FLOOR, old_adj - penalty), 3)
           new_fails += 1
           self._log_event(p["event_type"], p["best_action"], "stress_test",
           old_adj, new_adj, {"failures": failures, "notes": notes})
           results.append({
           "event_type": p["event_type"], "action": p["best_action"],
           "failures": failures, "penalty": round(old_adj - new_adj, 3),
           "notes": notes,
           })
   ...
   if not row:
    ...
    if row["revalidation_required"]:
     ...
     if (row["executions_since_revalidation"] or 0) >= self.REVALIDATION_AFTER_N:
      self._set_revalidation_required(event_type, action,
      f"{row['executions_since_revalidation']} execuções")
      return True
 ...
 def _set_revalidation_required(self, event_type: str, action: str, reason: str):
  ...
  self.conn.commit()
  self._log_event(event_type, action, "revalidation_required",
 ...
 def mark_revalidated(self, event_type: str, action: str):
  ...
  try:
   ...
   if not row:
    ...
    print(f" Padrão revalidado: {event_type} → {action} "
  ...
  if divergence < self.DIVERGENCE_TRIGGER:
   ...
   try:
    ...
    for p in rows:
     ...
     self.conn.execute(
     "UPDATE patterns_learned SET overconfidence_flagged=1 WHERE id=?", (p["id"],)
     )
     self._log_event(p["event_type"], p["best_action"], "overconfidence_alert",
     eff, eff,
     {"divergence_rate": divergence,
     "contradictions": p["human_contradiction_count"]})
     alerts.append({"event_type": p["event_type"], "action": p["best_action"],
     "confidence": eff, "divergence": divergence})
 ...
 def record_execution(self, event_type: str, action: str):
  ...
  h = datetime.now().hour
  hour_range = ("manhã" if 6 <= h < 12 else
  "tarde" if 12 <= h < 18 else
  "noite" if 18 <= h < 24 else "madrugada")
  try:
   self.conn.execute(
   """UPDATE patterns_learned
 ...
 def get_stats(self) -> dict:
  try:
   ...
   total = len(all_p)
   with_decay = sum(1 for p in all_p if
   p.get("confidence_adjusted") is not None and
   p.get("confidence_adjusted", 1) < p.get("success_rate", 1))
   ...
   if total == 0:
    ...
    try:
     ...
     return {
     "total": total, "with_decay": with_decay,
     "revalidation_required": need_reval, "overconfidence": overconf,
     "stress_failures": stress_fail, "reliability_score": score,
     "patterns": all_p, "events": events,
     }
 ...
 def print_report(self):
  ...
  if not s:
   ...
   return
   score_color = "" if s["reliability_score"] >= 80 else ("" if s["reliability_score"] >= 60 else "")
   print(f"\n {''*65}")
   print(f" Confidence Governance — Relatório")
   print(f" {''*65}")
   ...
   if s["patterns"]:
    ...
    for p in s["patterns"]:
     ...
     print(f" {p['event_type']:<28} {orig:>5.0%} {arrow}{adj:>5.0%} "
 ...
 def _log_event(self, event_type: str, action: str, kind: str,
 old_conf, new_conf, details: dict):
  try:
   self.conn.execute(
   ...
   details_json, ts)
   VALUES (?,?,?,?,?,?,?)""",
   (event_type, action, kind, old_conf, new_conf,
...
class DriftController:
   ...
   if row and row["t"] > 0:
    ...
    for er in ev_rows:
     if er["t"] >= 3:
      ...
      successful_corrections, success_rate, ts)
      VALUES (?,?,?,?,?,?,?)""",
      ("event_type", er["event_type"], days,
      er["t"], er["s"] or 0, r, _now())
      )
      self.conn.commit()
    ...
    if r7 is None or r30 is None:
     ...
     if delta > self.DRIFT_THRESHOLD:
      ...
      if alerts:
       ...
       f"Áreas: {names}\n"
       f"Use: python3 self_healing_engine.py --drift", "warn"
       )

       return alerts

 def _latest_snapshot(self, scope: str, scope_value: str,
 ...
 def register_canary(self, event_type: str, baseline_action: str,
 canary_action: str) -> int:
  ...
  print(f" Canary registrado #{canary_id}: {event_type} · "
 ...
 def record_canary_result(self, canary_id: int, success: bool,
 score: float) -> Optional[str]:
  ...
  row = self.conn.execute(
  "SELECT * FROM canary_tests WHERE id=?", (canary_id,)
  ).fetchone()
  if not row or row["canary_executions"] < self.MIN_CANARY_EXEC:
   ...
   self.conn.execute(
   "UPDATE canary_tests SET canary_rate=? WHERE id=?", (rate, canary_id)
   )

   baseline = row["baseline_rate"]
   if rate >= baseline + self.PROMOTE_MARGIN:
    decision = "passed"
    self._promote_canary(canary_id, row["event_type"],
    row["canary_action"], rate)
   elif rate <= baseline - self.ROLLBACK_MARGIN:
    decision = "failed"
    self._rollback_canary(canary_id, row["event_type"],
    row["canary_action"], row["baseline_action"],
    rate, baseline)
   else:
 ...
 def _promote_canary(self, canary_id: int, event_type: str,
 canary_action: str, rate: float):
  ...
  self.conn.commit()
  msg = (f" Canary #{canary_id} APROVADO\n"
  f"Tipo: {event_type} · Nova ação: {canary_action}\n"
  f"Taxa canary: {rate:.0%} vs baseline → promovido")
  print(f" {msg.splitlines()[0]}")
  _telegram_alert(msg, "info")

 def _rollback_canary(self, canary_id: int, event_type: str,
 canary_action: str, baseline_action: str,
 canary_rate: float, baseline_rate: float):
  ...
  self.conn.commit()
  msg = (f" Rollback #{canary_id}: {event_type}\n"
 ...
 def should_explore(self, event_type: str,
 current_action: str) -> tuple[bool, str]:
  """
 ...
 def record_exploration(self, event_type: str, action: str,
 ...
 def _maybe_propose_canary(self, event_type: str, explored_action: str):
  ...
  if not rows or rows["t"] < 3:
   ...
   if not bp:
    ...
    if (exp_rate >= baseline_rate + self.PROMOTE_MARGIN and
    bp["best_action"] != explored_action):
     # Não há canary ativo para este tipo?
     active = self.get_active_canary(event_type)
     if not active:
      print(f" Auto-canary proposto: {event_type} "
 ...
 def print_report(self):
  ...
  if not s:
   ...
   return
   print(f"\n {''*65}")
   print(f" System Drift Control — Relatório")
   print(f" {''*65}")
   ...
   if alerts:
    ...
    for a in alerts:
     ...
     print(f" ↘ {scope_name}: 7d={a['rate_7d']:.0%} vs 30d={a['rate_30d']:.0%} "
     f"(Δ={a['delta']:.0%})")
    else:
     print(f" Sem drift detectado")

     print(f"\n Canary tests:")
     print(f" Ativos: {s['active_canaries']} | "
     ...
     if canaries:
      ...
      for c in canaries[:8]:
       status_icon = {"active":"","passed":"","failed":"",
       "rolled_back":"↩","superseded":""}.get(c["status"],"•")
       print(f" {c['event_type']:<25} {c['baseline_action']:<18} "
       ...
       if exp:
        ...
        for e in exp:
         ...
         print(f" {e['action_tried']:<20} {e['t']:>4} tentativas "
         f"{rate:.0%} sucesso")
  ...
  if self.cycle_count % DriftController.SNAPSHOT_INTERVAL == 0 and self.cycle_count > 0:
   ...
   if drift_alerts:
    for da in drift_alerts:
     ...
     print(f" DRIFT [{scope}]: 7d={da['rate_7d']:.0%} vs "
     ...
     if decayed:
      ...
      if self.cycle_count % ConfidenceGovernance.STRESS_TEST_INTERVAL == 0 and self.cycle_count > 0:
       ...
       if self.cycle_count % ConfidenceGovernance.STRESS_TEST_INTERVAL == 0 and self.cycle_count > 0:
        ...
        if st_results:
         print(f" {len(st_results)} padrão(ões) penalizado(s) no stress test")
         ...
         if not anomalies:
          ...
          return {"cycle": self.cycle_count, "anomalies": 0, "resolved": 0}
          ...
          for anomaly in anomalies:
           ...
           self.conn, fp, anomaly["source"], ev_type,
           severity, details, product
           )
           if event_id is None:
            ...
            if attempts >= MAX_ATTEMPTS:
             print(f" Máx {MAX_ATTEMPTS} tentativas atingido — escalando")
             update_event(self.conn, event_id, status="escalated")
             _telegram_alert(
             f"Problema persistente após {MAX_ATTEMPTS} tentativas\n"
             f"Tipo: {ev_type}\nDetalhes: {details.get('message','')}", "critical"
             )
             escalated += 1
             continue
             ...
             if learned:
              learned_action, learned_rate, learned_score = learned
              # [10] Verificar se padrão exige revalidação antes de usar
              needs_reval = self.governance.check_revalidation(ev_type, learned_action)
              if needs_reval:
               print(f" Padrão '{learned_action}' requer revalidação "
               f"— não será usado automaticamente")
               learned = None # força diagnóstico normal
              else:
               print(f" Padrão aprendido: '{learned_action}' "
               f"(efetivo {learned_rate:.0%}, score {learned_score:.2f})")

               diagnosis = self.diagnostico.analyze(ev_type, severity, details, product)
               acao_diag = diagnosis.get("acao_recomendada", "notify_human")
               fonte = "claude" if ANTHROPIC_KEY else "rules"
               ...
               if learned and learned[0] != acao_diag:
                acao = learned[0]
                overridden = True
                print(f" ↺ Override: '{acao_diag}' → '{acao}' (aprendizado)")

                params = diagnosis.get("parametros_acao", {})
                conf = diagnosis.get("confianca_diagnostico", 0.0)
                causa = diagnosis.get("causa_provavel", "—")

                # [5] Registrar explicabilidade — ANTES de corrigir
                exp = self.explainability.record(
                self.conn, event_id, diagnosis, fonte,
                overridden=overridden, learned_action=acao
                )
                self.explainability.print_explanation(exp)

                update_event(self.conn, event_id,
                status="diagnosing",
                diagnosis_json=json.dumps(
                {**diagnosis, "explanation": exp}, ensure_ascii=False
                ))
                ...
                if severity == "critical":
                 ...
                 f"Anomalia crítica:\n{ev_type}\n"
                 f"Causa: {causa[:100]}\nAção: {acao}\n"
                 f"Confiança: {conf:.0%} | Fonte: {fonte}", "critical"
                 )

                 # CAMADA 8: HUMAN-IN-THE-LOOP
                 impacto = calcular_impacto(ev_type, acao, attempts, details)
                 nivel = impacto["nivel"]
                 nivel_icon = {"ALTO": "", "MEDIO": "", "BAIXO": ""}.get(nivel, "•")
                 fatores_str = ", ".join(impacto["fatores"]) or "sem fatores críticos"
                 print(f" [HITL] Impacto: {nivel_icon} {nivel} (score {impacto['score']}) — {fatores_str}")
                 ...
                 if nivel == "ALTO":
                  print(f" ALTO IMPACTO — bloqueando até aprovação humana")
                  update_event(self.conn, event_id, status="awaiting_approval")
                  approval_id = self.hitl.request_approval(
                  event_id, ev_type, impacto, diagnosis, acao, exp
                  )
                  hitl_status, hitl_acao = self.hitl.wait_for_response(approval_id)
                  ...
                  motivo_humano = ap_row["motivo"] if ap_row else ""
                  # [SLA] Registrar tempo de resposta humana
                  if ap_row and ap_row["responded_at"] and hitl_status != "timeout":
                   self.sla.record_human_response_sla(
                   event_id, ap_row["created_at"], ap_row["responded_at"]
                   )

                   acao_final_para_hdi = hitl_acao if hitl_acao else acao

                   if hitl_status == "rejected":
                    print(f" Correção rejeitada pelo operador — mantendo estado atual")
                    # [9] Registrar rejeição como decisão humana
                    human_decision_id = self.hdi.record_decision(
                    approval_id, event_id, ev_type, "rejected",
                    acao, "", # acao_final vazia = não executou
                    nivel, impacto["score"], motivo_humano
                    )
                    update_event(self.conn, event_id, status="rejected_human",
                    correction_applied="REJECTED_BY_HUMAN")
                    continue
                   else:
                    # approved / adjusted / timeout → vai executar
                    if hitl_status == "adjusted":
                     acao = hitl_acao or acao
                     print(f" Ação ajustada pelo operador: {acao}")
                    elif hitl_status == "timeout":
                     acao = hitl_acao # safe fallback
                     # approved → segue com acao original

                     # [9] Registrar decisão humana (approved/adjusted/timeout)
                     human_decision_id = self.hdi.record_decision(
                     approval_id, event_id, ev_type, hitl_status,
                     ...
                     if nivel != "ALTO":
                      active_canary = self.drift.get_active_canary(ev_type)
                      if active_canary and active_canary["canary_action"] != acao:
                       if self.drift.should_use_canary(active_canary):
                        acao = active_canary["canary_action"]
                        canary_id_drift = active_canary["id"]
                        print(f" [CANARY] Testando '{acao}' "
                        ...
                        should_exp, exp_action = self.drift.should_explore(ev_type, acao)
                        if should_exp:
                         acao = exp_action
                         was_exploration = True
                         print(f" [EXPLORE] Explorando ação '{acao}' "
                         f"({self.drift.EXPLORE_FRACTION:.0%} prob)")

                         # FASE 3: CORRIGIR — com SafetyGuard
                         # [6] Verificar permissão de segurança ANTES de agir
                         allowed, safety_reason = self.safety.can_apply(acao, ev_type)
                         if not allowed:
                          print(f" [3/4] BLOQUEADO pelo SafetyGuard: {safety_reason}")
                          update_event(self.conn, event_id, status="blocked_safety",
                          correction_applied=f"BLOCKED:{safety_reason}")
                          _telegram_alert(
                          f"SafetyGuard bloqueou correção\n"
                          f"Ação: {acao}\nMotivo: {safety_reason}", "warn"
                          )
                          continue

                          print(f" [3/4] Aplicando correção: {acao}...")
                          update_event(self.conn, event_id, status="correcting",
                          attempts=attempts + 1)

                          success, action_taken, before, after = self.correction.apply(
                          acao, params, details, event_id
                          )
                          update_event(self.conn, event_id,
                          ...
                          print(f" {'' if success else ''} {action_taken}")

                          # FASE 4: VALIDAR
                          print(f" [4/4] Validando resultado...")
                          time.sleep(3)

                          passed, score, notes = self.validator.validate(ev_type, before, after)
                          add_history(self.conn, event_id, attempts + 1, action_taken,
                          before, after, passed, notes)

                          final_status = "resolved" if (success and passed) else "retrying"
                          update_event(self.conn, event_id, status=final_status)

                          icon = "" if (success and passed) else ""
                          print(f" {icon} Validação: {notes} (score {score:.2f})")

                          # [6] Registrar no SafetyGuard (atualiza circuit breaker)
                          self.safety.record(acao, ev_type, success and passed, score)

                          # [7] Registrar no Learning (atualiza patterns_learned)
                          self.learning.record_outcome(ev_type, acao, success and passed, score)

                          # [9] Fechar loop HDI — registrar resultado da decisão humana
                          if approval_id is not None:
                           resultado_str = "resolved" if (success and passed) else "failed"
                           self.hdi.record_outcome(approval_id, resultado_str, score, self.learning)

                           # [10] Registrar execução na governança + verificar overconfidence
                           self.governance.record_execution(ev_type, acao)
                           self.governance.detect_overconfidence(self.hdi.get_stats())

                           # [SLA] Registrar tempo de correção vs target
                           if success and passed:
                            self.sla.record_correction_sla(event_id, ev_created, _now())

                            # [11] Registrar resultado de canary ou exploração
                            if canary_id_drift is not None:
                             decision = self.drift.record_canary_result(
                             canary_id_drift, success and passed, score
                             )
                             if decision:
                              print(f" Canary #{canary_id_drift}: {decision.upper()}")
                             elif was_exploration:
                              self.drift.record_exploration(ev_type, acao, success and passed, score)
                              ...
                              if success and passed:
                               ...
                               print(f" ↻ Tentativa {attempts+1}/{MAX_ATTEMPTS} — re-avaliado no próximo ciclo")

                               summary = {"cycle": self.cycle_count, "anomalies": len(anomalies),
                               "resolved": resolved, "escalated": escalated}

                               print(f"\n {''*60}")
                               print(f" Ciclo #{self.cycle_count}: {len(anomalies)} anomalias, {resolved} resolvidas, {escalated} escaladas")
                               return summary
...
class SLAMonitor:
 ...
 def get_stats(self) -> dict:
  ...
  for r in rows:
   ...
   "target_minutes": SLA_TARGETS.get(r["metric"].replace("_time", "_minutes")
   .replace("human_response", "human_response_minutes")
   .replace("correction_time", "correction_minutes"), 5),
   }
   # fill missing metrics
   for key, target_key in [("correction_time","correction_minutes"),
   ("human_response","human_response_minutes")]:
    if key not in stats:
     stats[key] = {"total": 0, "met": 0, "breached": 0, "compliance_pct": 100.0,
     "avg_minutes": 0, "target_minutes": SLA_TARGETS[target_key]}
     return stats
 ...
 def print_report(self):
  ...
  for metric, s in sorted(stats.items()):
   ...
   avg = s["avg_minutes"]
   print(f" {icon} {label} {pct:5.1f}% SLA | avg {avg:.1f}m | target ≤{target}m | {s['met']}/{s['total']} OK")
   print(f" {''*55}")
   ...
   print(f" {icon_ov} SLA GERAL: {overall:.1f}% ({total_met}/{total_all} eventos dentro do target)")
   print()
...
def generate_audit_report(conn: sqlite3.Connection) -> dict:
 ...
 stress_failures = conn.execute(
 "SELECT SUM(stress_test_failures) as n FROM patterns_learned"
 ).fetchone()["n"] or 0
 total_patterns = conn.execute(
 "SELECT COUNT(*) as n FROM patterns_learned"
 ).fetchone()["n"]
  ...
  for p in report.get("patterns_learned", [])[:5]:
   ...
   if sla:
    ...
    for metric, s in sla.items():
     icon = "" if s["compliance_pct"] >= 90 else ("" if s["compliance_pct"] >= 70 else "")
     print(f" {icon} {metric:<25} {s['compliance_pct']:>5.1f}% avg {s['avg_minutes']:.1f}m")
     print()
...
def show_status():
 if not DB_FILE.exists():
  ...
  print(f"\n {''*65}")
  print(f" Self-Healing Engine — Status Operacional")
  print(f" {''*65}")
  print(f" DB: {DB_FILE}")
  for c in counts:
   icon = {"resolved":"","escalated":"","retrying":"","detected":"","diagnosing":"","correcting":""}.get(c["status"],"•")
   ...
   if rows:
    ...
    for r in rows:
     print(f" {r['event_type']:<28} {r['severity']:<10} {r['status']:<12} {r['product']:<20} {r['attempts']}")
      ...
      for r in rate_rows:
       ...
       try:
        ...
        if drift_stats:
         ...
         print(f" {drift_icon} Drift: {len(alerts)} área(s) | "
         f"Canaries ativos: {act_c} | "
         f"Promovidos: {promo} | Rollbacks: {rolls}")
         print(f" (use --drift para detalhes)")
       except sqlite3.OperationalError:
        pass
        ...
        try:
         ...
         if gov_stats and gov_stats.get("total", 0) > 0:
          ...
          print(f" (use --governance para detalhes)")
         else:
         ...
         try:
          ...
          if hdi_stats and hdi_stats["total"] > 0:
           ...
           print(f" Decisões: {s['total']} total | "
           f" {s['aprovadas']} aprovadas | "
           f" {s['rejeitadas']} rejeitadas | "
           f" {s['ajustadas']} ajustadas")
           print(f" Acerto sistema: {s['taxa_acerto_sistema']:.0%} | "
          ...
          try:
           ...
           if pending_rows:
            ...
            for p in pending_rows:
             print(f" #{p['id']} [{p['impact_nivel']}] {p['ev_type']} → {p['acao_sugerida']}")
             print(f" Timeout: {p['timeout_at'][:19]}")
             print(f" → python3 self_healing_engine.py --approve {p['id']}")
            else:
...
def show_history():
 if not DB_FILE.exists():
  ...
  print(f"\n {''*65}")
  print(f" Histórico de Correções")
  print(f" {''*65}")
  if not rows:
   ...
   for r in rows:
    ...
    print(f" {icon} [{r['ts'][:16]}] {r['event_type']} (tent.{r['attempt']}) → {r['action'][:40]}")
    print(f" {r['notes']}")
    conn.close()
...
def main():
 ...
 parser.add_argument("--adjust", type=int, metavar="ID",
 help="Aprova aprovação #ID com ação diferente (requer --action)")
 parser.add_argument("--action", type=str, default="",
 help="Ação alternativa para --adjust")
 parser.add_argument("--motivo", type=str, default="",
 help="Razão da decisão (opcional, aprimora o aprendizado)")
 ...
 parser.add_argument("--revalidate", nargs=2, metavar=("TIPO", "ACAO"),
 help="Marca padrão como revalidado: --revalidate engine_bias_detected check_api")
 parser.add_argument("--drift", action="store_true",
 help="Relatório de System Drift Control")
 parser.add_argument("--canary", nargs=3, metavar=("TIPO","ATUAL","NOVO"),
 ...
 help="Gera relatório de auditoria JSON exportável")
 parser.add_argument("--proof-report", action="store_true",
        ...
        if args.governance:
         ...
         if args.drift:
          ...
          if args.canary:
           ...
           conn = _open_conn()
           DriftController(conn).register_canary(ev_type, baseline, canary_action)
           conn.close()
           return
           ...
           if args.sla:
            ...
            if getattr(args, "audit_report", False):
             ...
             if getattr(args, "proof_report", False):
              ...
              print(f" Abra no browser ou converta para PDF para compartilhar com clientes.")
              try:
               import subprocess
               ...
               if args.revalidate:
                ...
                ConfidenceGovernance(conn).mark_revalidated(ev_type, action)
                conn.close()
                return

                if args.approve is not None:
                 conn = _open_conn()
                 HumanApprovalGate(conn).respond(args.approve, "approved",
                 motivo=args.motivo)
                 conn.close()
                 return
                 ...
                 if args.reject is not None:
                  conn = _open_conn()
                  HumanApprovalGate(conn).respond(args.reject, "rejected",
                  motivo=args.motivo)
                  conn.close()
                  return
                  ...
                  if args.adjust is not None:
                   if not args.action:
                    print(" --adjust requer --action <acao> (ex: --adjust 3 --action retry)")
                    sys.exit(1)
                    conn = _open_conn()
                    HumanApprovalGate(conn).respond(args.adjust, "adjusted",
                    args.action, args.motivo)
                    conn.close()
                    return
