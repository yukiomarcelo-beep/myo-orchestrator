...
def run_scenarios(leads: float, conversion: float, ticket: float,
    ...
    for name, sc in scenarios.items():
        if name == "base":
            ...
            if base_profit != 0:
                sc["delta_lucro_pct"] = round((sc["profit"] - base_profit) / abs(base_profit) * 100, 1)
            else:
    ...
    return {
        "input": {"leads": leads, "conversion": conversion, "ticket": ticket, "monthly_cost": monthly_cost},
        "scenarios": scenarios,
        "ranking": ranking,
    }


def calc_ads_impact(investment: float, cpl: float,
                    conversion: float, ticket: float, monthly_cost: float) -> dict:
...
def _auto_decision(scenarios: dict, ranking: list) -> dict:
    ...
    return {
        "acao_prioritaria":  {"nome": prioridade,  "descricao": LABEL.get(prioridade, ""),  "dificuldade": DIFICULDADE.get(prioridade, "media"),  "impacto_pct": ranking[0][1]},
        "acao_secundaria":   {"nome": secundaria,   "descricao": LABEL.get(secundaria, ""),  "dificuldade": DIFICULDADE.get(secundaria, "media"),  "impacto_pct": ranking[1][1] if len(ranking) > 1 else 0} if secundaria else {},
        "acao_futura":       {"nome": futura,        "descricao": LABEL.get(futura, ""),       "dificuldade": DIFICULDADE.get(futura, "media"),       "impacto_pct": ranking[2][1] if len(ranking) > 2 else 0} if futura else {},
    }
...
async def run_simulation(leads: float, conversion: float, ticket: float,
                         monthly_cost: float, with_claude: bool = True) -> dict:
...
def show_result(r: dict):
    ...
    m_icon = "🟢" if base.get("margin_pct", 0) >= 70 else "🟡" if base.get("margin_pct", 0) >= 40 else "🔴"
    print(f"  Lucro/mês {m_icon}: R${base.get('profit', 0):,.2f}  ({base.get('margin_pct', 0):.1f}% margem)")
...
def show_history():
    ...
    print(f"  {'#':<3} {'Data':<17} {'Leads':>6} {'Conv':>6} {'Ticket':>8} {'Receita/mês':>14} {'Lucro/mês':>12} {'Margem':>8}")
    print("  " + "─" * 80)
...
async def main_async():
    ...
    r = await run_simulation(leads, conversion, ticket, cost,
