...
def _calculate(raw: dict) -> dict:
    ...
    # Churn Rate
    churn_rate = (
        (churned / total_customers) if total_customers > 0 else raw.get("churn_rate", 0) or 0
    )
...
async def run_saas_metrics(raw_input: dict) -> dict:
    ...
    print(
        f" CAC ${data['cac']:.2f} | LTV ${data['ltv']:.2f} | "
        f"Ratio {data['ltv_cac_ratio']:.1f}x {RATIO_ICON.get(data['status'],'')} {data['status'].upper()}"
    )
    print(
        f" Churn {data['churn_rate']*100:.1f}% {CHURN_ICON.get(data['churn_status'],'')} | "
    ...
    print(
        f" 12 meses → {proj_12.get('customers',0)} clientes | "
    ...
    if len(history) > 1:
        ...
        print(" Primeira sessão registrada")
    ...
    for path in sorted(glob.glob(f"{OUTPUTS_DIR}/saas_metrics_*.json"), reverse=True):
        try:
            ...
            if not records:
                ...
                print("" * 72)
                print(
                    f" {'#':<3} {'Produto':<22} {'LTV/CAC':>8} {'CAC':>8} {'LTV':>8} {'Churn':>7} {'MRR':>10} Status"
                )
                print(" " + "" * 68)
                ...
                for i, r in enumerate(records[:15], 1):
                    ...
                    print(
                        f" {i:<3} {r.get('product_name','?')[:20]:<22} "
                        f"{ratio:>6.1f}x "
                        f"${r.get('cac',0):>6.0f} "
                        f"${r.get('ltv',0):>6.0f} "
                                ...
                                for t in a["retention_tactics"]:
                                    ...
                                    if a.get("acquisition_efficiency"):
                                        ...
                                        if a.get("growth_path"):
                                            print(
                                                f" Caminho de crescimento: {a['growth_path'][:100]}"
                                            )

                                            # Projeção
                                            print("\n Projeção de Crescimento ")
                                            print(
                                                f" {'Período':<10} {'Clientes':>10} {'MRR':>12} {'ARR':>12}"
                                            )
                                            print(f" {''*48}")
                                            for k in ["3", "6", "12"]:
                                                sc = proj.get(k, {})
                                                print(
                                                    f" {k+'m':<10} {sc.get('customers',0):>10} ${sc.get('mrr',0):>11,.0f} ${sc.get('arr',0):>11,.0f}"
                                                )
                                                ...
                                                if len(hist) > 1:
                                                    ...
                                                    for h in hist:
                                                        ...
                                                        print(
                                                            f" {icon} {h['timestamp'][:15]} ratio {h['ltv_cac']:.1f}x "
