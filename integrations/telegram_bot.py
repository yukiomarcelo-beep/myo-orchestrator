...
try:
    ...
    def _guard_output(data, destination="telegram"):
        return True, "OK"
# ───────────────────────────────────────────────────────────────────────────────
...
async def cmd_retry(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        ...
        if not task:
            await update.message.reply_text(
                f"Task `{task_id}` não encontrada.", parse_mode="Markdown"
            )
            return
            if task.get("status") != "failed":
                await update.message.reply_text(
                    f"Task não está em status `failed` (atual: {task.get('status')}).",
                    parse_mode="Markdown",
                )
                return
    ...
    if not ctx.args:
        ...
        if not task:
            ...
            with open(path, "w", encoding="utf-8") as f:
                json.dump(task, f, ensure_ascii=False, indent=2)
                await update.message.reply_text(
                    f" Task `{task_id[:20]}` descartada.", parse_mode="Markdown"
                )
    ...
    if not BOT_TOKEN:
        ...
        if args.test:
            ...
            if not TELEGRAM_AVAILABLE:
                ...
                print(" MYO Bot iniciando (polling)...")
