...
def carregar_config() -> dict:
    defaults = {
        ...
        "projetos": {
            p: {"budget_diario_brl": 50.0, "budget_mensal_brl": 500.0} for p in PROJETOS_VALIDOS
        },
    }
    ...
    try:
        ...
        if (datetime.now() - dt).days > 30:
            print(
                f"⚠  [CostGuard] Preços com {(datetime.now()-dt).days} dias. Verifique anthropic.com/pricing"
            )
        return cfg
    except Exception:
        return defaults
...
def calcular_custo_usd(model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    return round(
        (input_tokens / 1_000_000 * preco["input"]) + (output_tokens / 1_000_000 * preco["output"]),
        6,
    )
    ...
    if nv_sol != nv_rec:
        if nv_sol > nv_rec:
            economia = round(
                (
                    1
                    - PRECOS_MODELO.get(recomendado, {}).get("output", 15)
                    / max(PRECOS_MODELO.get(modelo_solicitado, {}).get("output", 15), 1)
                )
                * 100
            )
...
def verificar_budget_diario(projeto: str):
    ...
    gasto = sum(
        r.get("custo_brl", 0)
        for r in registros
        if r.get("projeto") == projeto and r.get("timestamp", "")[:10] == hoje and not r.get("tipo")
    )
...
class CostGuard:
    ...
    def _registrar(
        self, projeto, tarefa, model, model_solicitado, resposta, duracao_ms, valor_contexto
    ):
        try:
            ...
            registro = {
                ...
                "tarefa": tarefa,
                "model": model,
                "model_solicitado": model_solicitado,
                "model_trocado": model != model_solicitado,
                "input_tokens": inp,
                "output_tokens": out,
                "total_tokens": inp + out,
                "custo_usd": custo_usd,
                "custo_brl": custo_brl_val,
                "duracao_ms": duracao_ms,
                "valor_contexto": valor_contexto,
...
def cmd_status():
    ...
    hoje = datetime.now().date().isoformat()
    registros_hoje = [
        r for r in registros if r.get("timestamp", "")[:10] == hoje and not r.get("tipo")
    ]
    print(f"\n{'━'*52}")
    ...
    for p, d in sorted(projetos.items(), key=lambda x: x[1]["custo_brl"], reverse=True):
        ...
        comp = (
            f"({'+' if d['custo_brl']>=media else ''}{((d['custo_brl']-media)/media*100) if media else 0:.0f}% vs média)"
            if media
            else "(sem histórico)"
        )
        ...
        flag = (
            " 🚨 BUDGET ESGOTADO"
            if d["custo_brl"] >= budget
            else (" ⚠  80% do budget" if d["custo_brl"] >= budget * 0.8 else "")
        )
        print(f"\n  {p}")
        print(f"    R$ {d['custo_brl']:.4f}  {comp}{flag}")
        print(
            f"    {d['execucoes']} execuções · {d['tokens']:,} tokens"
...
def cmd_report(dias=7):
    ...
    for r in registros:
        ...
        if r.get("model_trocado"):
            trocas += 1
    var = (
        f" ({'↑' if c_hoje > c_ontem else '↓'}{abs((c_hoje-c_ontem)/c_ontem*100):.0f}% vs ontem)"
        if c_ontem
        else ""
    )
    linhas = [
...
def cmd_audit():
    ...
    try:
        ...
        if diff > 0.10:
            ...
            print("  Verifique se todos os projetos usam guard.run()\n")
        else:
            print("\n  ✓ Cobertura completa\n")
    except Exception as e:
        print(f"  ⚠  Não foi possível buscar da Anthropic: {e}")
        print("  Verifique: console.anthropic.com/settings/usage\n")
