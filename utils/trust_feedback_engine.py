        ...
        for row in metrics.get("veto_rate_by_topic", []):
            ...
            if n < MIN_TOPIC_EVENTS:
                ...
                if vr >= VETO_RATE_CRITICAL:
                    ...
                    recs.append(
                        "Exigir pelo menos 1 fonte primária ou industry_report para claims deste tópico"
                    )
                    recs.append(
                        f"Revisar prompt do(s) engine(s) que mais geram claims de '{topic}'"
                    )
                    ...
                    if topic == "api_cost":
                        ...
                        level = FeedbackLevel.RECOMMEND
                        recs.append(
                            f"Monitorar '{topic}': veto_rate {vr:.0%} acima de {VETO_RATE_WARN:.0%}"
                        )
                        recs.append(
                            f"Considerar pedir fonte adicional para claims numéricas de '{topic}'"
                        )
                    else:
                        ...
                        if sq < 5.0:
                            recs.append(
                                f"Source quality média baixa ({sq}): encorajar fontes mais sólidas para este tópico"
                            )
        ...
        for row in metrics.get("critical_failure_rate_by_engine", []):
            ...
            if n < MIN_ENGINE_EVENTS:
                ...
                if cfr >= ENGINE_FAILURE_WARN:
                    level = FeedbackLevel.RECOMMEND
                    recs.append(
                        f"Revisar prompt de '{engine}' para separar melhor fato de inferência"
                    )
                    recs.append(
                        f"Adicionar instrução explícita para citar fontes numéricas em '{engine}'"
                    )
                    if vr_n > n * 0.4:
                        recs.append(
                            f"Alta taxa de validation_required ({vr_n}/{n}): considerar pré-filtro de qualidade no output deste engine"
                        )
                    else:
        ...
        for row in metrics.get("source_risk_index", []):
            ...
            if n < MIN_SOURCE_APPEARANCES:
                ...
                if veto_a >= SOURCE_RISK_WARN:
                    level = FeedbackLevel.RECOMMEND
                    recs.append(
                        f"Fontes do tipo '{stype}' estão associadas a {veto_a:.0%} dos vetos"
                    )
                    recs.append(
                        f"Reduzir peso efetivo de '{stype}' em tópicos críticos (pricing, competitor, api_cost)"
                    )
                    if score < 4:
                        recs.append(
                            f"Score médio baixo ({score}/10): considerar exigir fonte adicional quando '{stype}' for a única"
                        )
                    else:
                    ...
                    if benchmark_vr > 0.60 and research_cfr > 0.50:
                        ...
                        if api_cost_row:
                            ...
                            if api_vr > 0.65:
                                ...
                                launch_cfr = ctx_map.get("launch_ready", {}).get(
                                    "critical_failure_rate", None
                                )
                                research_cfr = ctx_map.get("research", {}).get(
                                    "critical_failure_rate", None
                                )
                                idea_cfr = ctx_map.get("idea", {}).get(
                                    "critical_failure_rate", None
                                )
                                        ...
                                        if q4_status in (
                                            ...
                                            benchmark_vr = next(
                                                (
                                                    r["veto_rate"]
                                                    for r in vbt
                                                    if r["topic"] == "competitor"
                                                ),
                                                0,
                                            )
                                            sri = metrics.get("source_risk_index", [])
                                            high_risk_src = any(
                                                r["risk_level"] == "alto" for r in sri
                                            )
                                            ...
                                            if aa_cfr is not None and aa_cfr > 0.30:
                                                ...
                                                checklist = {
                                                    "autonomous_agent_reduziu_critical_failure": aa_cfr
                                                    is not None
                                                    and aa_cfr <= 0.20,
                                                    "fallback_permaneceu_baixo": aa_fallback_rate
                                                    is not None
                                                    and aa_fallback_rate < 0.15,
                                                    "benchmark_block_sem_travar_research": q2_status
                                                    not in (
                                                        "bloqueando_demais_em_research",
                                                        "sem_dados",
                                                    ),
                                                    "api_cost_block_mais_preciso": q3_status
                                                    == "calibrado",
                                                    "divergencia_clara_entre_contextos": q4_status
                                                    == "divergencia_clara"
                                                    and (divergence_delta or 0) >= 0.25,
                                                    # volume mínimo por contexto = 8 eventos (consistente com threshold de divergência)
                                                    "amostra_suficiente_por_contexto": len(
                                                        [r for r in fbc if r["total_claims"] >= 8]
                                                    )
                                                    >= 2,
                                                }
                                                ...
                                                ready_for_6c = (
                                                    ...
                                                    and criteria_met >= 3
                                                    and unknown_rate_local
                                                    <= 0.10  # integridade mínima do contexto
                                                )

                                                # Verificar se unknown_context_rate está alto o suficiente para bloquear a 6C
                                                fbc_unknown = next(
                                                    (r for r in fbc if r["context"] == "unknown"),
                                                    None,
                                                )
                                                unknown_n_local = (
                                                    fbc_unknown["total_claims"]
                                                    if fbc_unknown
                                                    else 0
                                                )
                                                total_local = (
                                                    sum(r["total_claims"] for r in fbc)
                                                    if fbc
                                                    else 0
                                                )
                                                unknown_rate_local = (
                                                    round(unknown_n_local / total_local, 3)
                                                    if total_local
                                                    else 0
                                                )
                                                ...
                                                for r in fbec:
                                                    ...
                                                    if r["context"] == "unknown":
                                                        ...
                                                        blockers = []
                                                        if (
                                                            aa_fallback_rate is not None
                                                            and aa_fallback_rate >= 0.20
                                                        ):
                                                            blockers.append(
                                                                "fallback_alto_no_autonomous_agent"
                                                            )
                                                            if q4_status == "sem_dados":
                                                                blockers.append(
                                                                    "pouca_amostra_por_contexto"
                                                                )
                                                                if not checklist[
                                                                    "divergencia_clara_entre_contextos"
                                                                ]:
                                                                    blockers.append(
                                                                        "sem_divergencia_suficiente_entre_contextos"
                                                                    )
                                                                    if unknown_rate_local > 0.10:
                                                                        blockers.append(
                                                                            "high_unknown_context_rate"
                                                                        )
            ...
            if r["context"] == "unknown":
                ...
                if unknown_rate > 0.10 and total >= MIN_EVENTS_FOR_LEARNING:
                    breakdown_str = (
                        ", ".join(
                            f"{eng}={pct:.0%}"
                            for eng, pct in sorted(
                                unknown_engine_rates.items(), key=lambda x: -x[1]
                            )
                        )
                    ...
                    if vr_n > 0.5:
                        ...
                        vr_n + exp < 0.1
                        and metrics.get("total_claims", 0) >= MIN_EVENTS_FOR_LEARNING
                    ):
                        report.alerts.append(
                            Alert(
                                level=FeedbackLevel.OBSERVE,
                        ...
                        for a in observe_alerts:
                            ...
                            if report.claim_watchlist:
                                ...
                                for w in report.claim_watchlist:
                                    ...
                                    lines.append(
                                        f"- [{w['recurrences']}x] {w['question'][:80]}{flag}"
                                    )
                                    ...
                                    if cpr:
                                        ...
                                        lines.append("| Pergunta | Status |")
                                        lines.append("|----------|--------|")
                                        lines.append(
                                            f"| Q1 — autonomous_agent obediente ou só fallback? | `{cpr.get('q1_autonomous_agent_status', '—')}` (cfr={cpr.get('q1_aa_cfr','—')}, fallback={cpr.get('q1_aa_fallback_rate','—')}) |"
                                        )
                                        lines.append(
                                            f"| Q2 — benchmark_block segurando ou bloqueando demais? | `{cpr.get('q2_benchmark_block_status', '—')}` |"
                                        )
                                        lines.append(
                                            f"| Q3 — api_cost_block preciso após patch? | `{cpr.get('q3_api_cost_block_status', '—')}` |"
                                        )
                                        lines.append(
                                            f"| Q4 — divergência clara por contexto? | `{cpr.get('q4_context_divergence_status', '—')}` (delta={cpr.get('q4_divergence_delta','—')}) |"
                                        )
                                                ...
                                                if ucr > 0:
                                                    ...
                                                    if blockers:
                                                        ...
                                                        for b in blockers:
                                                            ...
                                                            with open(
                                                                FEEDBACK_MD, "w", encoding="utf-8"
                                                            ) as f:
                                                                f.write("\n".join(lines))
                                                                print(
                                                                    f"[trust_feedback] Summary MD salvo: {FEEDBACK_MD}"
                                                                )
        ...
        if report.data_warning:
            ...
            unknown_alert = next(
                (
                    ...
                    if a.category == FeedbackCategory.GLOBAL and a.subject == "execution_context"
                ),
                None,
            )
            if unknown_alert:
                        ...
                        if report.claim_watchlist:
                            ...
                            for w in report.claim_watchlist[:5]:
                                ...
                                if cpr:
                                    ...
                                    print(f"\n 6C Context-Aware Policy: {label} ")
                                    print(
                                        f" Q1 autonomous_agent : {cpr.get('q1_autonomous_agent_status','—')}"
                                        f" (fallback={cpr.get('q1_aa_fallback_rate','—')})"
                                    )
                                    print(
                                        f" Q2 benchmark_block : {cpr.get('q2_benchmark_block_status','—')}"
                                    )
                                    print(
                                        f" Q3 api_cost_block : {cpr.get('q3_api_cost_block_status','—')}"
                                    )
                                    print(
                                        f" Q4 divergência : {cpr.get('q4_context_divergence_status','—')}"
                                    ...
                                    if rch:
                                        ...
                                        print(
                                            f" Critérios atendidos : {cpr.get('criteria_met',0)}/6"
                                        )
                                        ucr = cpr.get("unknown_context_rate", 0)
                                        if ucr > 0:
                                            ube = cpr.get("unknown_context_by_engine", {})
                                            ...
                                            flag = " BLOQUEADOR" if ucr > 0.10 else ""
                                            print(
                                                f" Context integridade : unknown={ucr:.0%}{flag} [{ube_str}]"
                                            )
                                            if cpr.get("blockers"):
                                                print(
                                                    f" Bloqueadores : {', '.join(cpr['blockers'])}"
                                                )
