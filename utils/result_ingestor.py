...
Uso:
 python3 -m utils.result_ingestor           → processa todos os não-ingeridos
...
# Guard: o map só pode apontar para contextos válidos
assert set(CONTEXT_MAP.values()).issubset(VALID_EXECUTION_CONTEXTS), (
    f"CONTEXT_MAP contém valores inválidos: "
    f"{set(CONTEXT_MAP.values()) - VALID_EXECUTION_CONTEXTS}"
)


def _map_context(raw: str) -> str:
    """Converte experiment_context da task para execution_context da verification."""
    return CONTEXT_MAP.get(str(raw).lower(), DEFAULT_EXECUTION_CONTEXT)
