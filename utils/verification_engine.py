...
def normalize_execution_context(value: str) -> str:
    """
    Normaliza e valida execution_context.

    - "" | None  -> DEFAULT_EXECUTION_CONTEXT (retrocompat)
    - valido     -> lowercase normalizado
    - invalido   -> ValueError (fail-fast)
    """
    if value is None or value == "":
        return DEFAULT_EXECUTION_CONTEXT
    normalized = str(value).strip().lower()
    if normalized not in VALID_EXECUTION_CONTEXTS:
        raise ValueError(
            f"execution_context invalido: {value!r}. "
            f"Valores aceitos: {sorted(VALID_EXECUTION_CONTEXTS)}"
        )
    return normalized
...
async def _claude(prompt: str, max_tokens: int = 2000) -> dict:
    """Chama Claude API e retorna dict JSON parseado."""
    if not ANTHROPIC_API_KEY:
        print("[verification] AVISO: ANTHROPIC_API_KEY não configurada. Retornando stub.")
        return {}
    ...
    try:
        ...
        print(f"[verification] Erro chamando Claude: {e}")
        return {}
...
def should_block_benchmark_claim(
    ...
    """Veto específico pra claims de benchmark/métrica com evidência fraca."""
    if num_type != "benchmark":
        return False
    # Requer fonte forte E confiança alta
    if source_quality < 7.0:
        return True
    if source_count < 1:
        return True
    if confidence < 75:
...
def should_block_api_cost_claim(
    ...
    """Veto específico pra claims de custo de API — precisa de fonte oficial."""
    if topic != "api_cost":
        return False
...
def _prompt_classify_claims(text: str, context: str = "") -> str:
    """Gera o prompt que o Claude usa pra classificar claims do texto."""
    ctx_block = f"\nContexto: {context}\n" if context else ""
    return f"""Analise o texto abaixo e classifique cada afirmação objetiva.
{ctx_block}
Texto:
\"\"\"
{text}
...
{{
  ...
  "claims": [
    {{
      ...
      "validation_question": "pergunta aberta se requires_validation=true"
    }}
  ],
  "assumptions": ["suposição implícita 1", "..."],
  "requires_validation": ["pergunta aberta geral 1", "..."]
}}
"""
...
class VerificationEngine:
    ...
    async def verify(
        ...
        """
        Analisa texto via Claude, classifica afirmações e retorna VerificationResult.

        execution_context: estágio do negócio (idea | research | mvp | launch_ready | scaling).
        Valor vazio -> DEFAULT_EXECUTION_CONTEXT. Valor inválido -> ValueError.
        """
        ...
        # Qualidade de fontes via source_policy
    ...
    def generate_validation_tasks(
        ...
        candidates = [p for p in result.unverified_claims if p.requires_validation]
        if not candidates:
            return tasks
        ...
        for topic, packs in groups.items():
            ...
            questions = [p.validation_question for p in packs if p.validation_question]
