...
class TrustAggregator:
    ...
    def _source_risk(self, events: list[dict]) -> list[dict]:
        ...
        result = []
        for stype in sorted(
            type_counts, key=lambda s: type_vetoed[s] / type_counts[s], reverse=True
        ):
            n = type_counts[stype]
        result.append(
            {
    ...
    def _validation_recurrence(self, events: list[dict]) -> list[dict]:
        ...
        return [{"question": c["representative"], "count": c["count"]} for c in clusters[:15]]
    ...
    def _save_summary(self, metrics: dict):
        ...
        for row in fbc:
            divergence = (
                " " if row["critical_failure_rate"] > 0.5 and row["total_claims"] >= 8 else ""
            )
        lines.append(
    ...
    def print_dashboard(self):
        ...
        for row in sri:
            print(
                f" {row['source_type']:<22} risco={row['risk_level']:<6} score_médio={row['avg_score']}"
            )
        ...
        flag = (
            " ← DIVERGÊNCIA"
            if row["critical_failure_rate"] > 0.5 and row["total_claims"] >= 8
            else ""
        )
        print(
            f" {row['context']:<14} {bar} {row['critical_failure_rate']:.0%} n={row['total_claims']}{flag}"
        )
        ...
        print(
            f" {row['engine']:<25} [{row['context']:<12}] {bar} {row['critical_failure_rate']:.0%}{fb}"
        )
