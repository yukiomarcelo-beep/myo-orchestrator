...
try:
    ...
    def _record_cost(ti, to, m="claude-sonnet"):
...
class _TrackContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if cost_status["status"] in ("shutdown_non_critical", "emergency_stop"):
            ...
            print(
                f"[Security/Observability] ALERTA CUSTO: {cost_status['status']} — "
                f"${cost_status['daily_total']:.4f} hoje",
                file=sys.stderr,
            )
